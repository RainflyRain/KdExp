...
#include "../../../libs/KudanCV/include/KudanCV.h"

#define TAG "LightSnail From JNI" // 这个是自定义TAG标识
...
static KudanMatrix3 gK;

// Project the point (0,0,0) using the camera intrinsics and extrinsics. Also need to pass in the image with (see function)
static KudanVector3 gPosition;
static KudanQuaternion gOrientation;
bool flag;

void Java_eu_kudan_ar_input_JavaInput_initialiseImageTracker(
        JNIEnv *env,
        jobject /* this */,
        jstring key,
        jint width,
...
void Java_eu_kudan_ar_input_JavaInput_initialiseArbiTracker(
        JNIEnv *env,
        jobject /* this */,
        jstring key,
        jint width,
...
void Java_eu_kudan_ar_input_JavaInput_startArbiTracker(
        JNIEnv *env,
        jobject /* this */,
...
void Java_eu_kudan_ar_input_JavaInput_stopArbiTracker(
        JNIEnv *env,
        jobject /* this */) {
...
jboolean Java_eu_kudan_ar_input_JavaInput_addTrackableToImageTracker(
        JNIEnv *env,
        jobject /* this */,
        jobject bitmap,
        jstring name) {
...
jfloatArray Java_eu_kudan_ar_input_JavaInput_processImageTrackerFrame(
        JNIEnv *env,
        jobject /* this */,
        jbyteArray image,
        jint width,
    ...
    LOGD("processImageTrackerFrame start");
    ...
    if (trackedList.size() == 1) {
        ...
        float w = tracked->getWidth();//1635
        float h = tracked->getHeight();//1113.843750
//        LOGE("tracked->w h %f %f ",w,h);
//        LOGE("tracked->x y z  %f %f %f ",position.x,position.y,position.z);
//        LOGE("tracked->quaterion x,y,z,w %f %f %f  %f ",orientation.x,orientation.y,orientation.z,orientation.w);
...
jfloatArray Java_eu_kudan_ar_input_JavaInput_getK(
        JNIEnv *env,
        jobject /* this */)
{
    //float Kdata[9];
    ...
    env->SetFloatArrayRegion(result, 0, 9, gK.data);

    return result;
}

jfloatArray Java_eu_kudan_ar_input_JavaInput_getPosition(
        JNIEnv *env,
        jobject /* this */)
{
    if(flag == false){
    ...
    PositionData[0] = gPosition.x;
    PositionData[1] = gPosition.y;
    PositionData[2] = gPosition.z;

    jfloatArray result = env->NewFloatArray(3);

    env->SetFloatArrayRegion(result, 0, 3, PositionData);

    return result;
}

jfloatArray Java_eu_kudan_ar_input_JavaInput_getOrientation(
        JNIEnv *env,
        jobject /* this */)
{
    //float Kdata[9];
    ...
    jfloatArray result = env->NewFloatArray(9);
    //env->SetFloatArrayRegion(result, 0, 9, gOrientation.toRotation().data);

    KudanMatrix3 rotationMatrix(gOrientation);
    env->SetFloatArrayRegion(result, 0, 9, rotationMatrix.data);

    return result;
}
jfloatArray Java_eu_kudan_ar_input_JavaInput_getOrientationQuaterion(
        JNIEnv *env,
        jobject /* this */)
{
    //float Kdata[9];
    ...
    float quaternion[4];
    quaternion[0] = gOrientation.x;
    quaternion[1] = gOrientation.y;
    quaternion[2] = gOrientation.z;
    quaternion[3] = gOrientation.w;

    jfloatArray result = env->NewFloatArray(4);

    env->SetFloatArrayRegion(result, 0, 4, quaternion);

    return result;
}

jfloatArray Java_eu_kudan_ar_input_JavaInput_processArbiTrackerFrame(
        JNIEnv *env,
        jobject /* this */,
        jbyteArray image,
